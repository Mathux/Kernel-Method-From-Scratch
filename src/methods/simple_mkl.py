    ...
    def fit(self, verbose=False):
     ...
     #       print("weights:", weights)
            ...
            for m in self.vrange(self.param.n_iter_fit, desc=desc):
                ...
                #print("D_os:", D_os)
                J_os, grad_J_os, _, _ = self.compute_J_grad_J_D(K_os, mu, d_os)

                if J_os >= J:
                    break
#            print("gamma_max = ", gamma_max)
#            print("D normalized", D)
...
#            weights = weights/np.sum(weights)
            ...
            print("quad_max = ", quad_max)
            print("quad = ", quad(K,alpha))
            gap = quad_max - quad(K, alpha)
            print('gap = ', gap)
            print('weights = ', weights)
            print('D = ', D)
            print('gamma = ', gamma)
            if np.abs(gap) < self.param.tol:
                break
...
if __name__ == '__main__':
    ...
    data = AllSeqData(parameters= {"nsmall" : 500, "small" : True})[0]["train"]

    from src.kernels.multikernel import MultiKernel
    
    parameters = {
        "kernels": ["spectral"]*15,
        "_parameters": [{"k": i, "sparse": True} for i in range(6, 21)][::-1]
    }
